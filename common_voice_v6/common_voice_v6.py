...
class CommonVoiceV6(tfds.core.GeneratorBasedBuilder):
  ...
  def _split_generators(self, dl_manager: tfds.download.DownloadManager):
    ...
    archive_path = dl_manager.manual_dir / (self.builder_config.language + '.tar')
    ...
    clip_folder = os.path.join(extracted_path, "cv-corpus-6.1-2020-12-11", self.builder_config.language, "clips")
    return [
        tfds.core.SplitGenerator(  # pylint: disable=g-complex-comprehension
            name=k,
            gen_kwargs={
                "audio_path": clip_folder,
                "label_path": os.path.join(extracted_path, "cv-corpus-6.1-2020-12-11", self.builder_config.language, "%s.tsv" % v)
            },
  ...
  def _generate_examples(self, audio_path, label_path):
    ...
    with tf.io.gfile.GFile(label_path) as file_:
      ...
      for i, row in enumerate(dataset):
        file_path = os.path.join(audio_path, row["path"])
